...
class ChatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextViewDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(handleConnectedUserUpdateNotification), name: "userWasConnectedNotification", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(handleDisconnectedUserUpdateNotification), name: "userWasDisconnectedNotification", object: nil)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(handleUserTypingNotification), name: "userTypingNotification", object: nil)
    ...
    func handleUserTypingNotification(notification: NSNotification) {
        if let typingUsersDictionary = notification.object as? [String: AnyObject] {
            ...
            if totalTypingUsers > 0 {
                ...
                lblOtherUserActivityStatus.text = "\(names) \(verb) now typing..."
    ...
    func showBannerLabelAnimated() {
        UIView.animateWithDuration(0.75, animations: { () -> Void in
            self.lblNewsBanner.alpha = 1.0
                ...
                self.bannerLabelTimer = NSTimer.scheduledTimerWithTimeInterval(2.0, target: self, selector: #selector(self.hideBannerLabel), userInfo: nil, repeats: false)
        }
    }
