...
class GeotificationsViewController: UIViewController, AddGeotificationsViewControllerDelegate, MKMapViewDelegate, CLLocationManagerDelegate {
  ...
  func addGeotificationViewController(controller: AddGeotificationViewController, didAddCoordinate coordinate: CLLocationCoordinate2D, radius: Double, identifier: String, note: String, eventType: EventType) {
    ...
    startMonitoringGeofencing(geotification)
  ...
  func locationManager(manager: CLLocationManager, monitoringDidFailForRegion region: CLRegion?, withError error: NSError) {
    showSimpleAlertWithTitle("Error", message: "Monitoring Failed for region with identifier: \(region?.identifier)", viewController: self)
  }
  
  func locationManager(manager: CLLocationManager, didFailWithError error: NSError) {
    showSimpleAlertWithTitle("Error", message: "Location Manager failed with the following error: \(error)", viewController: self)
  }
  ...
  func startMonitoringGeofencing(geotification: Geotification) {
    ...
    if CLLocationManager.authorizationStatus() != .AuthorizedAlways {
      showSimpleAlertWithTitle("Warning", message: "Your geotification is saved but will only be activated once you grant Geotify permission to acess the device location", viewController: self)
    }
